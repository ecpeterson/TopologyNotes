...
\chapter{Homotopy Types and Exact Sequences}\label{ExactSeqsChapter}

Two overall goals in this text are:
\begin{enumerate}
    \item
    To treat topological objects as if they were algebraic, dissecting and assembling them by methods analogous to those we use with, e.g., modules.
    \item
...
\begin{itemize}
    ...
    For example, \define{simplicial sets} and \define{Kan complexes} are two such combinatorial options.
    Though we won't use them, \define{varieties} are a popular purely algebraic notion of ``space''.
    }
    This eases the import of algebraic operations into this new context, but it requires one to link the chosen alternative definition with a traditional notion of a space.%
    \item
    ...
    By supposing that these tools are ``shadows'' of---or perhaps are themselves---the desired operations which act directly on spaces, we examine their formal properties to see what manner of algebra we are dealing with.
...
The tools we develop in this chapter and the language in which we cast them will become the fundamental methods of dissection and assembly used in all later chapters.
...
We first set up the basics of category theory and introduce a few example categories, which amounts to setting the stage on which the rest of the course will play out.
Since one of our express intentions is do very little geometry or ``point-set'' topology, much of this Lecture will concern itself with boldly asserting such facts recast in category-theoretic language, so that they can be guiltlessly referred to later.

We begin with some basic operations that one may perform on spaces.
The categorical perspective is to understand objects in a category not by intrinsic presentations, like how the points in a space are arranged, but by their extrinsic effects, like how they relate to other objects in the category.
...
\begin{description}
    ...
    \todo{Remark about opens and unique extension to closures?  Of course, this requires a separability hypothesis on the target.}
    \item[Quotients]\marginnote{\citep[0.3]{Switzer}}
    For $\sim$ an equivalence relation on $X$, there is a space $X / \sim$ such that the set $\CatOf{Spaces}(X / \sim, Y)$ bijects with the subset \[\{f \in \CatOf{Spaces}(X, Y) \mid x \sim x' \Rightarrow f(x) = f(x')\}\] of those continuous maps which are constant on the partition given by $\sim$.
    ...
    Equivalently, maps in $\CatOf{Spaces}(X/A, T)$ correspond exactly to those maps in $\CatOf{Spaces}(X, T)$ which are constant on (nonempty) $A$.
    ...
    * \arrow{r} \arrow[bend right]{rrd} & X / A \arrow[densely dotted, "\exists!"]{rd} \\
    ...
    \end{tikzcd}
    \end{center}
    Adopting this diagram instead as the \emph{definition}, this yields $X / \emptyset = X \sqcup *$, which authors sometimes posit as an unmoored ``convention''.
\end{description}
...
\begin{description}
    ...
    For a fixed space $Y$, the coproduct and gluing properties characterize the functor $\CatOf{Spaces}(-, Y)$ \define{represented} by $Y$ as a \define{sheaf}, i.e., a functor $\CatOf{Spaces}^{\op} \to \CatOf{Sets}$ which preserves limits (for certain covers).
    ...
    Our affection for representable functors will grow substantially in the coming sections, and we will find relief in \Cref{BrownRepSec}, which gives a representability criterion adapted to homotopy theory.
    }
\end{description}
...
\noindent
These constructions also interact well with each other in most cases of geometric interest.
For example:
...
\begin{definition}
The category of \define{relative pairs} has objects given by inclusions $A \subseteq X$, and a morphism $(A \subseteq X) \to (B \subseteq Y)$ between two such pairs is given by a continuous map $f\co X \to Y$ with the property $f(A) \subseteq B$.
...
The category $\CatOf{Spaces}_{*/}$ of \define{pointed spaces}
\footnote{%
This notation is to be read as ``spaces under the singleton'' or ``spaces with a specified map from the singleton''.
Namely, the map $* \to X$ selects $x_0$.
}%
has objects given by $\{x_0\} \subseteq X$ for some choice of singleton $\{x_0\}$, and a morphism $(\{x_0\} \subseteq X) \to (\{y_0\} \subseteq Y)$ between two such pointed spaces is a map $f\co X \to Y$ with the property $f(x_0) = y_0$.
Pointed spaces and their maps arise when defining reduced homology or homotopy groups (e.g., the fundamental group).
Pointed spaces form a full subcategory of relative pairs: they are the special case where the privileged subset is a singleton.
\end{definition}
...
In many cases, if one forgets about the privileged subset and considers just the underlying object of $\CatOf{Spaces}$, the construction even agrees with that in $\CatOf{Spaces}$.
...
\todo{Convert the margin note about adjunctions into the main text?  One could then also comment on currying as an example of an adjunction.}
...
\marginnote{%
For the reader who knows about limits: we didn't have to quotient this time because of the chirality of the limits involved.
Namely, coproducts and quotients are both colimits, but a product is a limit.
}%
...
\todo{Define a monoidal structure?}
...
Finally, we introduce the \define{homotopy category of (pointed) spaces}, $h\CatOf{Spaces}_{*/}$, with the ``$h$'' denoting ``homotopy''.
Its objects are the same as that of $\CatOf{Spaces}_{*/}$ but whose morphism sets are given by the quotient \[h\CatOf{Spaces}_{*/}(X, Y) = \frac{\CatOf{Spaces}_{*/}(X, Y)}{f \sim g},\] where $f \sim g$ indicates that there is a homotopy $H\co X \sm I_+ \to Y$ between $f$ and $g$.
The objects are this category are often referred to as \define{homotopy types}.
The reduced morphism sets mean that many maps in $\CatOf{Spaces}$ acquire inverses when considered as maps in $h\CatOf{Spaces}$, e.g., the inclusion $* \to \R$.
...
Because we will work in this category so often, we will abbreviate this mapping set to \[h\CatOf{Spaces}_{*/}(X, Y) = [X, Y].\]
\end{definition}
...
With categorical definitions in place, we are in position to investigate the traditional tools of algebraic topology in these terms.
...
\todo{Remark on non-Cartesian monoidal structures?}
\end{definition}
...
If $G$ is a group object in a category $\CatOf{C}$, then $\CatOf{C}(-, G)$ is a functor from $\CatOf{C}^{\op}$ \todo{Definition of $\op$?} to $\CatOf{Groups}$.
\end{lemma}
\begin{proof}
...
\todo{I remmeber $[Y, X]$ being convenient notation in past contexts.  Review this whole doc to see if it would fit here too.}
\end{example}
...
In order to address this by the same tactic, we must confront the structure on $X$ which makes $[X, Y]$ a group.
...
For $K$ an $H$--cogroup, $[K, -]$ is valued in groups. \qed
\marginnote{%
...
\todo{Generally, these pictures are buggy.}
\begin{center}
...
\begin{center}
    ...
    ,
\end{center}
and coassociativity is checked by noting that the pulled-back color maps can be rotated into one another.
...
Let $S$ be a set with two products $\circ$ and $*$ which share a unit $e$ and which satisfy the distributive law: \[(x * x') \circ (y * y') = (x \circ y) * (x' \circ y').\]  Then the products agree, and both are associative and commutative.
\end{lemma}
\begin{proof}
\todo{We don't show associativity!}
To show commutativity, we cleverly redistribute:
\begin{align*}
...
A composable pair of maps of groups \[N \xrightarrow f G \xrightarrow g H\] is called \define{exact} when $\im f = \ker g$.
...
Today we are after something similar: we would like to study when continuous maps $X \xrightarrow f Y \xrightarrow g Z$ induce exact sequences of the functors they co/represent.

\begin{definition}\marginnote{\citep[{Definitions 2.30 and 2.49}]{Switzer}}
If the sequence of pointed sets \[[-, X] \xrightarrow{f_*} [-, Y] \xrightarrow{g_*} [-, Z]\] is exact, then we say that the underlying sequence of spaces is \define{exact}.
If the sequence of pointed sets \[[X, -] \xleftarrow{f^*} [Y, -] \xleftarrow{g^*} [Z, -]\] is exact, then we say that the underlying sequence of spaces is \define{coexact}.%
\marginnote[-5\baselineskip]{%
These more commonly go by the names \define{fiber} and \define{cofiber} sequences of spaces.
}
\end{definition}

At first brush, one might imagine that such sequences are somewhat rare, or that they at least require some nice properties of the spaces $X$, $Y$, $Z$ or of the maps $f$, $g$.%
\marginnote[-\baselineskip]{%
This scarcity is true for maps of groups: $f\co H \to G$ can only participate in an exact sequence when $\im f$ is normal.
It is a good exercise to check that the following constructions for spaces do not violate this.
}
...
\end{proof}
\todo{Mention moduli objects?---moduli of maps + homotopies?}
...
\todo{Remove numbering from margin.}

There are also dual results for exact sequences: any map of spaces participates in an (infinite) exact sequence extending to the left.
...
\end{tikzcd}
\end{center}
Again, this square is not a pullback in $h\CatOf{Spaces}_{*/}$, but $PY = Y^I$ is contractible, earning this object the name \define{homotopy fiber}.
(Note that while the pointed function space $Y^I$ is contractible, the unpointed function space of maps from $I$ to $Y$ is weakly equivalent to $Y$.)
}
\end{lemma}
\begin{proof}[Proof sketch]
Again, the construction of $P_f$ rests on building into it the data of a null-homotopy.
\todo{Another opportunity for a description in terms of a moduli problem.}
...
\todo{Move this into the next section as motivation?}
...
\todo{Motivate this with $H^*(X, A) = H^*(X \cup CA)$.  $P_f$ seems poorly adapted to $H^*$ but well-adapted to $\pi_*$.}
The construction $P_f$ from the previous lecture is a little mysterious.
To close out this Chapter, we will explore it in two lights\todo{``Two lights'': bad, awkward}: today and a bit later on.
...
\begin{lemma}\marginnote{\citep[Definition 3.8]{Switzer}}\pushQED{\qed}
The exact continuation $P_i$ of $i$ is given by the function object \[P_i = (X, A, x_0)^{(I, \partial I, 0)}. \qedhere \popQED\]
\end{lemma}
...
\todo{Describe these maps in terms of restrictions.}
...
\todo{Usually ``weak equivalence'' refers to a map.  Immediately get in the habit; there's nothing special about pairs here.}
\end{definition}
...
\todo{Forward reference to an analogous statement for the $P$ construction.}
...
\todo{This belongs earlier, maybe even just the start of this section.}
...
It is extremely productive to see how far this analogy can be pushed: what theorems about homology can be replicated for homotopy?
When a theorem for homology fails for homotopy, is it at least partially recoverable?
What is the discrepancy?
\todo{Emphasize that homology is computable because it has these theorems, and homotopy has them too!}
...
The highlighted sequence is exact.
\end{lemma}
\begin{proof}[Proof sketch]
...
In contrast to the small and polite model for the coexact continuation $C_f$, the presentation of the exact continuation $P_f$ as a function space feels large and unwieldly.%
\marginnote[-2\baselineskip]{
...
In this section and the next, we explore a more familiar context in which spaces \emph{like} $P_f$ arise, as well as conditions under which these model exact continuations.

To get off the ground, recall the whole point of the design of $P_f$ was its participation in the long exact sequence%
\marginnote{\Cref{LexseqInvolvingP}}
\[\cdots \to \pi_2 Y \to \pi_1 P_f \to \pi_1 X \to \pi_1 Y \to \pi_0 P_f \to \pi_0 X \to \pi_0 Y.\]
...
has the \define{homotopy lifting property} with respect to a space $T$ when for all solid diagrams
\begin{center}
\begin{tikzcd}
T \arrow["f"]{r} \arrow["- \times 0"]{d} & E \arrow["p"]{d} \\
T \times I \arrow["H"]{r} \arrow[densely dotted, "{\widetilde H}"]{ru} & B
\end{tikzcd}
\end{center}
...
For $PX = X^I$ the pointed path space, evaluation $\mathit{ev}\co PX \to X$ gives a fibration with fiber $\Loops X$.
\end{example}
...
For $p\co E \to B$ a fibration with subspaces $B' \subseteq B$ and $E' := p^{-1}(B') \subseteq E$, the natural map $\pi_{\ge 1}(E, E') \to \pi_{\ge 1}(B, B')$ is an isomorphism.
\end{corollary}
\begin{proof}
...
\marginnote[-\baselineskip]{%
Specifically, we avail ourselves of a pointed self-homeomorphism of $I \times I$ which restricts to a homeomorphism of $U$ with $I \times \{0\}$.
}

To dispatch the other homotopy groups, we use the equivalences
\todo{It is confusing to put the subspace on the left in this notation for a pair but on the right in the usual relative homology / homotopy notation.  Be consistent, use $\supseteq$?}
\begin{center}
\begin{tikzcd}
...
\begin{corollary}\label{PiStarLexseqForFibration}\marginnote{\citep[4.7]{Switzer}}
...
\todo{Didn't this already hold for $P_f$?  Why is this here?}
...
A \define{fiber bundle with fiber $F$} is a map $p\co E \to B$ such that $B$ admits an open cover $\{U_\alpha\}_\alpha$ with local homeomorphisms $\phi_\alpha\co U_\alpha \times F \to p^{-1}(U_\alpha)$, each participating in a commuting triangle
\begin{center}
\begin{tikzcd}
U_\alpha \times F \arrow["\mathrm{proj}"]{rd} \arrow["\phi_\alpha"]{rr} & & p^{-1}(U_\alpha) \arrow["p"]{ld} \\
& U_\alpha.
\end{tikzcd}
\end{center}
\end{definition}
...
The most common source of these ``local projections'' are the \define{homogeneous spaces} of Lie theory: a quotient map of a Lie group by a subgroup has everywhere-isomorphic fibers, and in good cases the coset space can be arranged to admit a suitable open cover.
...
\end{lemma}
\begin{proof}
\todo{May need to discuss this in terms of $G$--bundles to meet requirements of next section.}
...
Consider the subgroup $O(n) \subseteq O(n+k)$ of block matrices \[\left\{\left[\begin{array}{c|c} O(n) & 0 \\ \hline 0 & I \end{array} \right]\right\} \le O(n+k).\]
...
\todo{Add a concrete example for Grassmannians, like projective space.}
\end{example}

\begin{example}\marginnote{\citep[Examples 4.14.4, 5, 7]{Switzer}}
These linear algebraic examples do not rest on $\mathbb R$.
One can build analogues of these examples with $U(n)$ and $\C^n$, or even with $\Sp(n)$ and $\H^n$.
\end{example}
...
\todo{This base is extremely unexciting.  Should probably do Spin too.}
\end{example}

\begin{example}\marginnote{\citep[Proposition 4.15]{Switzer}}
Applying \Cref{PiStarLexseqForFibration} to \Cref{SOtoOisAFiberBundle} yields \[\pi_{\ge 1} SO(n) \cong \pi_{\ge 1} O(n).\]
\end{example}
...
\Cref{PiStarLexseqForFibration} gives an exact sequence of pointed sets \[0 \to \pi_1 S^1 \to \pi_0 \Z \to 0.\]
\end{example}
...
\end{proof}
\todo{From here \emph{up} to ``A discrete fiber...'' can be demoted to a condensed remark.}
...
Since $\Z \le \R$ is a closed subgroup, $\Z$ acting on $\R$ by $1 \cdot x = x+1$ is properly discontinuous.
...
\todo{Mention the $\pi_1$ action somewhere in all of this.  Specifically, $G$ acting on $\pi_1 X/G$.}
\todo{I have scrawled ``mention that this is a \emph{group} quotient!'', but I don't know in reference to what.}
...
Theory of infinitesimal embeddings

$G$--bundles

Associated bundle construction for $\Aut F$ and $h\Aut F$

Prop 11.44 states that $O(n)$ is a deformation retraction of $GL_n(\mathbb R)$
