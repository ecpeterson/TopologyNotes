...
\todo{Consider using a different symbol for a generic homology theory.}
Applying a homology theory $h_*$ gives the \define{Serre spectral sequence}.
\end{definition}

\begin{theorem}[Serre]
\marginnote{There is a version of this without the hypothesis on $\pi_1 B$, where ``local coefficients'' / ``twisted homology'' are used.  We won't need it.}
If $\pi_1 B$ acts trivially on $h_* F$,%
\marginnote{For instance, $\pi_1 B$ acts trivially on $h_* F$ if $\pi_1 B = 0$.}
...
\todo{Add a signature for the differential in this grading.}
...
\marginnote{Part of the fun of spectral sequences in algebraic topology, as opposed to their appearances in other fields like algebraic geometry, is that we don't just filter our problems away into differentials---we then compute the differentials.}
...
\begin{remark}\label{EdgeHomDefn}
...
F \arrow{r} \arrow[equal]{u} & E \arrow{r} \arrow[equal]{u} & E \cup CF \arrow["f"]{u}
\end{tikzcd}
\end{center}
is a $2n$--equivalence.
...
% }
We have previously computed\todo{Really?} $H^*(K(\F_2, 1); \F_2) \cong \F_2[x]$, where $|x| = 1$.
